...
Com a evolução do projeto para a nuvem confirmada, manter dois caminhos de deploy deixou de compensar: gera duplicação de manifests (`k8s/03-app/service.yaml` NodePort, do kind, vs. `k8s/aws/service-loadbalancer.yaml` LoadBalancer, da AWS — pasta separada só porque o Terraform do kind aplicava genericamente todo `.yaml` de `03-app/`), duplicação de Terraform (`infra/` local vs. os Repositórios 2/3 reais) e um `ci-cd.yml` que descreve um comportamento de "produção" (`deploy-hml`/`deploy-prod`) que não reflete o deploy real do projeto.
...
- **`k8s/02-mysql/*`**, **`k8s/01-config/mysql-configmap.yaml`**, **o `service.yaml` (NodePort) do kind** — o banco agora é sempre o RDS do Repositório 3, e o Service exposto é sempre o `LoadBalancer` que antes vivia em `k8s/aws/`.
- A pasta **`k8s/aws/`** deixa de existir: sem o glob do Terraform do kind, não há mais motivo para separar os manifests da AWS dos demais. `service-loadbalancer.yaml` → `k8s/03-app/service.yaml`, `secret-aws.yaml.example` → `k8s/secret.yaml.example` (raiz, junto dos outros templates de referência).
...
- `k8s/` fica mais simples e achatado: `00-namespace/`, `01-config/app-configmap.yaml`, `03-app/` (`deployment.yaml`, `hpa.yaml`, `service.yaml`) e `secret.yaml.example` na raiz — sem pasta por ambiente, já que só existe um.
