...
- Na Fase 3, esse caminho **não foi substituído** pelo deploy real na AWS, e sim mantido em paralelo ([ADR 0010](0010-additive-deploy-paths.md)) — continua sendo a validação gratuita de cada push, independente de qualquer sessão do AWS Academy Learner Lab estar ativa. Será removido quando o deploy real for testado de ponta a ponta.
