...
Este é o deploy real contra a infraestrutura provisionada nos Repositórios [2](https://github.com/Adriana-Meyer/fiap-tech-challenge-kubernetes-infrastructure) (EKS) e [3](https://github.com/Adriana-Meyer/fiap-tech-challenge-database-infrastructure) (RDS): o workflow **`deploy-aws.yml`**, disparado manualmente (`workflow_dispatch`), testado com sucesso de ponta a ponta. A simulação local com kind (acima) serviu como validação prévia, gratuita e independente de sessão do Lab, durante o desenvolvimento — com o deploy real validado, a decisão é manter só o caminho AWS; a remoção do kind (`infra/`, `k8s/02-mysql/*` e os jobs correspondentes em `ci-cd.yml`) é tratada numa branch/PR dedicada (ver [ADR 0010](adr/0010-additive-deploy-paths.md)).
...
- Usa `k8s/aws/service-loadbalancer.yaml` (Service `LoadBalancer`) em vez de `k8s/03-app/service.yaml` (NodePort, específico do kind). Esse manifesto fica fora de `00-namespace`/`01-config`/`02-mysql`/`03-app` de propósito: o `infra/manifests.tf` do caminho kind aplica **todo** `.yaml` dentro dessas 4 pastas via `fileset`, e um Service duplicado com o mesmo nome (`workshop-app`) dentro de `03-app/` quebrava o `apply` do kind com "resource already exists".
- O Secret `app-secret` é criado via `kubectl create secret generic` a partir de GitHub Secrets, incluindo `SPRING_DATASOURCE_URL`/`_USERNAME`/`_PASSWORD` apontando para o RDS — ver [`k8s/aws/secret-aws.yaml.example`](../k8s/aws/secret-aws.yaml.example) para a estrutura esperada. Os valores de `RDS_DATASOURCE_URL`/`RDS_USERNAME`/`RDS_PASSWORD` chegam automaticamente como Secrets deste repositório: o `terraform-apply.yml` do Repositório 3 faz `terraform output` e envia via `gh secret set` direto para cá (nunca aparecem em log) — não é preciso copiar nada manualmente.
