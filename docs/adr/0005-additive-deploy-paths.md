...
Antes da Fase 3, o pipeline de CI/CD (`ci-cd.yml` + `deploy.yml`) já sobe e derruba um cluster **kind** efêmero dentro do próprio runner do GitHub Actions, a cada push, para validar o deploy de ponta a ponta sem custo de nuvem. A Fase 3 pede deploy real contra o EKS (Repositório 2) e o RDS (Repositório 3).
