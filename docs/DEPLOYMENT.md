...
- O Secret `app-secret` é criado via `kubectl create secret generic` a partir de GitHub Secrets, incluindo `SPRING_DATASOURCE_URL`/`_USERNAME`/`_PASSWORD` apontando para o RDS — ver [`k8s/aws/secret-aws.yaml.example`](../k8s/aws/secret-aws.yaml.example) para a estrutura esperada. Os valores de `RDS_DATASOURCE_URL`/`RDS_USERNAME`/`RDS_PASSWORD` são copiados manualmente do `terraform output` do Repositório 3, mesmo padrão manual já usado para os outros secrets.
